...
public class PageInterceptor implements Interceptor {
	...
	private String getPageSql(Page<?> page, String sql) {
		...
		if (page.getGroupBy().size() != 0) {
			sqlBuffer = getGroupBy(page, sqlBuffer);
		}
		if (!StringUtil.isEmpty(page.getSidx())) {
			sqlBuffer = getOrderSql(page, sqlBuffer);
		}
	...
	private StringBuffer getGroupBy(Page<?> page, StringBuffer sqlBuffer) {
		sqlBuffer = getSelectByGroupBy(page, sqlBuffer);
		String groupBySql = getGroupBySql(page);
		sqlBuffer.append(groupBySql);
		return sqlBuffer;
	}
	
	/**
	 * 在有group by语句时，将select语句的字段进行修改(如果调用此处，返回的vo中必须有一个count属性进行对应)
	 * @param page
	 * @param sqlBuffer
	 */
	private StringBuffer getSelectByGroupBy(Page<?> page, StringBuffer sqlBuffer) {
		StringBuffer sqlSelectBuffer = new StringBuffer();
		sqlSelectBuffer.append("select ");
		initGroupByColumns(page, sqlSelectBuffer);
		sqlSelectBuffer.append(", count(0) as count ");
		int index = sqlBuffer.indexOf("from");
		sqlSelectBuffer.append(sqlBuffer.substring(index));
		return sqlSelectBuffer;
	}
	 ...
	 * @return
	 */
	private String getGroupBySql(Page<?> page) {
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append(" group by ");
		initGroupByColumns(page, sqlBuffer);
		return sqlBuffer.toString();
	}
	
	/**
	 * 将groupBy集合里面的字段进行拼接，用于group by后面的字段拼接和select后面的字段拼接
	 * @param page
	 * @param sqlBuffer
	 */
	private void initGroupByColumns(Page<?> page, StringBuffer sqlBuffer) {
		List<String> groupBy = page.getGroupBy();
		int i = 0;
		int length = groupBy.size();
	...
	private StringBuffer getOrderSql(Page<?> page, StringBuffer sqlBuffer) {
	...
	private void setTotalRecord(Page<?> page, MappedStatement mappedStatement,
		...
		try {
			...
			// 此处如果存在group by的条件，最终返回的数据应为数据集的数量；如果没有条件，则将返回值存入totalRecord中
			if (page.getGroupBy().size() != 0) {
				int i = 0;
				while (rs.next()) {
	...
	private String getCountSql(Page<?> page, String sql) {
		...
		if (page.getGroupBy().size() != 0) {
			String groupBySql = getGroupBySql(page);
			return "select count(0) " + sql.substring(index) + groupBySql;
		} else {
