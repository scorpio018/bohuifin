                        ...
                        <div class="col-xs-12">
                            ...
                            <div>
                                <span class="ziti">关键字：</span><input type="text" id="keyword" value="${requestScope.keyword}"
                                                                     placeholder="按管理员名或者真实姓名查询" style="width:200px;" class="ziti">
                                &nbsp;&nbsp;&nbsp;&nbsp;
                                ...
                                <a href="javascript:void(0)" class="btn btn-sm btn-primary" style="height:28px;" onclick="add();">
                                    <p class="ziti" style="margin-top:-3px;">新建管理员</p>
                                </a>&nbsp;&nbsp;
...
<script type="text/javascript">
    ...
    function initAjax() {
        jQuery(grid_selector).jqGrid({
            url : "admin/managers/managerData",
            ...
            autowidth : true,
            colNames : ['管理员编号','管理员名','真实姓名', '昵称', '排序','操作'],
            colModel : [
                {name:'managerId',index:'manager_id', width:40,key:true,align:'center'},
                {name:'managerName',index:'manager_name', width:40,align:'center'},
                ...
                {name:'managerId',index:'manager_id', width:240, fixed:true,resize:false,title:false,sortable:false,
                    formatter : function(cellvalue, options, rowObject) {
                        ...
                        tmp += "<a href='javascript:void(0);' onclick='del(\""+cellvalue+"\", \"" + rowObject.managerName + "\");' title='删除' >删除</a>";
                        return tmp+"</div>";
                    }
                }
            ],
    ...
    function initpwd(managerId) {
        $.ajax({
            url : "admin/managers/initpwd",
            type : "POST",
            data : {
                managerId : managerId
            },
    ...
    function edit(managerId) {
        ...
        window.location.href="<%=SystemConst.BASE_PATH%>admin/managers/showAddManager?managerId=" + managerId + "&pageNo="+pageNo+"&pageSize="+pageSize + "&act=edit&keyword=" + keyword;
    }
    ...
    function add() {
        ...
        var pageSize = $(grid_selector).jqGrid("getGridParam", "rowNum");
        var keyword = $("#keyword").val();
        window.location.href = "<%=SystemConst.BASE_PATH%>admin/managers/showAddManager?pageNo="+pageNo+"&pageSize="+pageSize + "&act=add&keyword=" + keyword;
    }

    function del(managerId, managerName) {
        var pageNo = $(grid_selector).jqGrid("getGridParam", "page");
        var pageSize = $(grid_selector).jqGrid("getGridParam", "rowNum");
        var keyword = $("#keyword").val();
        swal({
            title : "确认删除【" + managerName + "】?",
            type : "warning",
            ...
            $.ajax({
                ...
                success : function(data) {
                    if (data == "success") {
                        window.location.href = "<%=SystemConst.BASE_PATH%>admin/managers/delManager?managerId=" + managerId + "&pageNo=" + pageNo + "&pageSize=" + pageSize + "&keyword=" + keyword;
                    } else {
                        swal({
                            title : data,
                            type : "warning",
        ...
        if (sel.length) {
            swal({
                ...
                for (var i=0;i<sel.length;i++) {
                    selIds.push(sel[i]);
                    window.location.href="<%=SystemConst.BASE_PATH%>admin/managers/delManager?managerIds="+selIds + "&keyword=" + keyword;
                }
            });
        } else {
            swal({
