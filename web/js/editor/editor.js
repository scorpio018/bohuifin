...
(function( $ ){
	...
	var methods = {
					...
					id:"tblForm"+idExtn 
						...
						class:"row" 
							...
							class:"col-xs-12 col-sm-6 col-md-6 col-lg-6"
								...
								).append($('<option/>',{ text:"请选择", value:""}
								)).append($('<option/>',{ text:"居左", value:"left"}
								)).append($('<option/>',{ text:"中间", value:"center"}
								)).append($('<option/>',{ text:"居右",	value:"right"}))
								...
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellspacing"+idExtn,	text:"单元格之间的空间"}
							)).append($('<input/>',{
								id:"tblCellspacing"+idExtn,
								type:"text", 
								class:"form-control form-control-width",
								value:1
							})).append($('<label/>',{ for:"tblCellpadding"+idExtn,	text:"单元边沿与其内容之间的空白"}
							)).append($('<input/>',{
		...
		{
			var fonts = { "<span style='font-family:宋体'>宋体</font>"	 : "宋体",
						  "<span style='font-family:仿宋'>仿宋</font>"	 : "仿宋",
						  "<span style='font-family:黑体'>黑体</font>"	 	 : "黑体",
						  "<span style='font-family:楷体'>楷体</font>"	 	 : "楷体",
						  "<span style='font-family:隶书'>隶书</font>"	 	 : "隶书",
						  "<span style='font-family:微软雅黑'>微软雅黑</font>": "微软雅黑",
						  "<span style='font-family:新宋体'>新宋体</font>"  : "新宋体",
						  "<span style='font-family:幼圆'>幼圆</font>"	 : "幼圆",
						  "<span style='font-family:Arial'>Arial</font>"	 	 : "Arial",
						  "<span style='font-family:Comic Sans MS'>Comic Sans MS</font>"  : "Comic Sans MS",
						  "<span style='font-family:Courier New'>Courier New</font>" : "Courier New",
						  "<span style='font-family:Open Sans'>Open Sans</font>" 	 : "Open Sans",
						  "<span style='font-family:Tahoma'>Tahoma</font>":"Tahoma",
						  "<span style='font-family:Verdana'>Verdana</font>":"Verdana"};

			var styles = {  "<h1>标题 1</h1>":"<h1>",
							"<h2>标题 2</h2>":"<h2>",
							"<h3>标题 3</h3>":"<h3>",
							"<h4>标题 4</h4>":"<h4>",
							"<h5>标题 5</h5>":"<h5>",
			...
			var fontsizes = {	"<font size='1'>1</font>"	:"1",
			...
			var menuItems = { 'fonteffects': true,
						   ...
						   'insert_link': { "modal": true,
											...
											"onSave":function(){
												...
												if(!targetURL.match(urlPattern)){
													methods.showMessage.apply(editorObj,["errMsg","请输入合法链接地址"]);
													return false;
												}													
												if(range=='' && targetText==''){ 
						   ...
						   'insert_img'	: { "modal": true,
											...
											"tooltip": "插入图片", 
											"modalHeader": "插入图片（只能插入.gif .jpg .png .jpeg文件）",
						...
						'insert_table'	: { "modal": true,
											...
											"onSave": function(){
												...
												if(!tblRows.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","表格行必须为一个正整数"]);
													return false;
												}													
												if(!tblColumns.match(intReg)){
													methods.showMessage.apply(this,["tblErrMsg","表格列必须为一个正整数"]);
													return false;
												}
												if(tblWidth!="" && !tblWidth.match(cssReg)){
													methods.showMessage.apply(this,["tblErrMsg","请输入一个正整数或者一个合法的CSS测量单位 (px,em,ex,%,in,cm,mm,pt,pc)"]);
													return false;
												}
												...
												if(tblBorder!="" && !tblBorder.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","表格边框必须为一个正整数"]);
													return false;
												}
												if(tblCellspacing!="" && !tblCellspacing.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","单元格之间的空间必须为一个正整数"]);
													return false;
												}
												if(tblCellpadding!="" && !tblCellpadding.match(numReg)){
													methods.showMessage.apply(this,["tblErrMsg","元边沿与其内容之间的空白必须为一个正整数"]);
													return false;
												}
			...
			var $this = $(this).hide();	       	
	       	...
	       	var menuBar = $( "<div/>",{ id : "menuBarDiv",
								  		class : "row-fluid"
							}).prependTo(containerDiv).css("text-align","center");
	       	var editor  = $( "<div/>",{	class : "Editor-editor",
										css : {overflow: "auto",height:settings['editorHeight']},
										contenteditable:"true"
						 	}).appendTo(containerDiv).html($(this).html());
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").html('<div class="label">'+'字数 : '+charCount+'</div>');
            	});
	        }	        
		...
		createImageContext: function(event,cMenuUl){
			...
			var onSave = function(){
				...
				if(imageTarget!=""&& !imageTarget.match(urlPattern)){
					methods.showMessage.apply(this,["imageErrMsg","请输入合法的图片地址"]);
					return false;
				}
				if($("#imgHidden").val()!=""){
			...
			var modalId="editProperties";
       		...
       		var onSave = function(){ 
				...
				if(tblWidthEdt!="" && !tblWidthEdt.match(tblEdtCssReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","请输入一个正整数或者一个合法的CSS测量单位 (px,em,ex,%,in,cm,mm,pt,pc)"]);
					return false;
				}
				...
				if(tblBorderEdt!="" && !tblBorderEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","表格边框必须为一个正整数"]);
					return false;
				}
				if(tblCellspacingEdt!="" && !tblCellspacingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","单元格之间的空间必须为一个正整数"]);
					return false;
				}
				if(tblCellpaddingEdt!="" && !tblCellpaddingEdt.match(tblEdtNumReg)){
					methods.showMessage.apply(this,["tblErrMsgEdt","单元边沿与其内容之间的空白必须为一个正整数"]);
					return false;
				}
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				menuWrapElement.append($('<a/>',{
										...
										}).html(itemSettings["default"]).append($("<i/>",{class:"icon-angle-down icon-on-right"})).mousedown(function(e){
											e.preventDefault();
										}));
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn btn-sm'});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])
					menuElement.addClass(itemSettings["icon"]);
				else
